...
class SuperSpeedAnalysisFromDomain:
    ...
    def open_mult_line_split(self,path_to_file, delim):
        
        with open(path_to_file, 'r') as inputfile:
            data = [x.strip().split(delim) for x in inputfile]
        return data
    ...
    def go(self):
        ...
        for domain_list, genome_in_domain, ng_size in zip(just_neigh_data, genomes_with_domain, neigh_genome_size):
            ...
            if counter != 100:
                for fine_domain in alls:
                    ...
                    dloc_dglob_part.append((str(dlok),str(dglob)))
                big_data.append(part)
                dloc_dglob_big.append(dloc_dglob_part)
            ...
            if counter != 100 and domain_list == just_neigh_data[-1]:
                ...
                for place,domain in enumerate(alls):
                    data_to_save = [x[place] for x in dloc_dglob_big]
                    data_ref = [x[place] for x in big_data]
                    with open('/home/djangoadmin/final_site-project/scripts/temp_data/' + domain+"_diff", 'a+') as output:
                        for tupl, ref in zip(data_to_save, data_ref):
                            if ref != '0.0':
                                output.write(','.join(tupl))
                                output.write("\n")
        ...
        for i in alls:
            ...
            data_dlok_dglob = self.open_mult_line_split("/home/djangoadmin/final_site-project/scripts/temp_data/" + i + "_diff", delim=",")
            # LOCAL
            ...
            dlok_list = [float(x[0]) for x in data_dlok_dglob]
            dglob_list = [float(x[1]) for x in data_dlok_dglob]
            scores.append((wynik.pvalue, mean))
            densities.append((statistics.mean(dlok_list), statistics.mean(dglob_list)))
        ...
        for domena, wynik,dens in zip(alls, scores_after_correction,densities):
            if self.user_distance == 0 or self.skip_negative == "no":
                if wynik[1] > 0:
                    ...
                    najlepsze_dane.at[domena, 'Dloc'] = dens[0]
                    najlepsze_dane.at[domena, 'Dglob'] = dens[1]
                else:
        ...
        for domain_list, genome_in_domain, ng_size in zip(just_neigh_data, genomes_with_domain, neigh_genome_size):
            ...
            if counter != 100:
                for fine_domain in alls:
                    ...
                    diff = dlok - dglob
                    part.append(str(diff))
                    dlok_dglob_part.append((str(dlok), str(dglob))) # CHANGE
                big_data.append(part)
                dlok_dglob_big.append(dlok_dglob_part) # CHANGE
            ...
            if counter != 100 and domain_list == just_neigh_data[-1]:
                ...
                for place,domain in enumerate(alls):
                    data_to_save = [x[place] for x in dlok_dglob_big]
                    data_ref = [x[place] for x in big_data]
                    with open('/home/djangoadmin/final_site-project/scripts/temp_data/' + domain+"_diff", 'a+') as output:
                        for tupl, ref in zip(data_to_save, data_ref):
                            if ref != '0.0':
                ...
                for fine_domain in alls:
                    ...
                    diff = dlok - dglob
                    part.append(str(diff))
                    dlok_dglob_part.append((str(dlok), str(dglob)))
                big_data.append(part)
                dlok_dglob_big.append(dlok_dglob_part)
