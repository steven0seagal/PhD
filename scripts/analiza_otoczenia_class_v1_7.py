...
class NeighbourhoodAnalyzer():
    ...
    def __init__(self,user_PfamDomain,user_DistanceValue, user_OrganismValue,user_OutputValue ):
    ...
    def OpenDirectory(self, file_name):
        file_names=[]
        with open("/home/djangoadmin/final_site-project/important_files/"+file_name +'.txt', 'r') as f:
...
#####################################################################################################
    def SaveComplete(self,file, message_for_output,message_for_additional_data, message_down,complete_data):
        with open('/home/djangoadmin/final_site-project/media/'+file+'.txt' , 'w') as output_file:
            ...
            output_file.write(message_down)
            output_file.write("\n")
            output_file.write(str(message_for_additional_data))
            output_file.write("\n")
    ...
    def OpenDatabaseFile(self,file_name):
        plik=[]
        with open("/home/djangoadmin/final_site-project/important_files/database_file/"+file_name+".txt") as inputfile:
    ...
    def OpenFile(self,file_name):
        plik=[]
        with open("/home/djangoadmin/final_site-project/important_files/"+file_name+".txt") as inputfile:
            for line in inputfile:
                plik.append(line.strip().split())
        return plik
...
##################################################################################################
    def CollectAllData(self, final_list, genome_counter, neigh_counter, genome_number_overall,genome_number_to_stat):
        ...
        for pfam in final_list:
            ...
            part = []
            part.append(domena)
            part.append(final_list[counter][1])
            counter+=1
            #pdb.set_trace()
            #pdb.set_trace()
            try:
...
################################################################################
    ...
    def GO(self):
        ...
        message_for_additional_data = ["Pfam domain", "PVALUE",  "sum in neighbourhood", "average in neighbourhood" ,"min in neighbourhood","max in neighbourhood", "sum in genome", "average in genome", "min in genome", "max in genome"]
    ...
    ################################################################################
        ...
        #     tax = self.OpenDirectory("salmonella")
        # elif self.user_OrganismValue ==  "pseudomonas":
        #     tax = self.OpenDirectory("pseudomonas")
        # elif self.user_OrganismValue ==  "staphylococcus":
        #     tax = self.OpenDirectory("staphylococcus")
        # elif self.user_OrganismValue ==  "streptococcus":
        #     tax = self.OpenDirectory("streptococcus")
        # elif self.user_OrganismValue ==  "mycobacterium":
        #     tax = self.OpenDirectory("mycobacterium")
        # elif self.user_OrganismValue ==  "acinetobacter":
        #     file_names = self.OpenDirectory("acinetobacter")
        # elif self.user_OrganismValue ==  "vibrio":
        #     file_names = self.OpenDirectory("vibrio")
        # elif self.user_OrganismValue ==  "bacillus":
        #     file_names = self.OpenDirectory("bacillus")
        # elif self.user_OrganismValue ==  "streptomyces":
        #     file_names = self.OpenDirectory("streptomyces")
        # elif self.user_OrganismValue ==  "klebsiella":
        #     file_names = self.OpenDirectory("klebsiella")
        # elif self.user_OrganismValue ==  "enterococcus":
        #     file_names = self.OpenDirectory("enterococcus")
        # elif self.user_OrganismValue ==  "burkholderia":
        #     file_names = self.OpenDirectory("burkholderia")        
        # elif self.user_OrganismValue ==  "lactobacillus":
        #     file_names = self.OpenDirectory("lactobacillus")
        # elif self.user_OrganismValue ==  "campylobacter":
        #     file_names = self.OpenDirectory("campylobacter")
        # elif self.user_OrganismValue ==  "helicobacter":
        #     file_names = self.OpenDirectory("helicobacter")
        # elif self.user_OrganismValue ==  "shigella":
        #     file_names = self.OpenDirectory("shigella")
        # elif self.user_OrganismValue ==  "alldb":
        #     file_names = self.OpenDirectory("alldb")
...
#        self.SaveOutputAddData(self.user_OutputValue,additional_data,message_for_additional_data,message_down)
        self.SaveComplete(self.user_OutputValue, message_for_output,message_for_additional_data, message_down,complete_data )
