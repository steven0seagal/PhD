...
class NeighborhoodAnalyzerFromGene:
    ...
    def go(self):
        ...
        for i in alls:
            ...
            percentage.append(len(data_to_calculate) / len(just_neigh_data))
        ...
        najlepsze_dane = pd.DataFrame(columns=['PVALUE', 'occurence in neighborhoods',
                                               ...
                                               'In what percentage',
                                               'Family', 'Summary'])
        for domena, wynik, single_percentage in zip(alls, scores, percentage):
            if self.user_distance_value == 0:
                if wynik[1] > 0:
                    najlepsze_dane.at[domena, 'PVALUE'] = wynik[0]
                    najlepsze_dane.at[domena, 'Density difference'] = wynik[1]
                    najlepsze_dane.at[domena, 'In what percentage'] = single_percentage
                else:
    ...
    def go(self):
        ...
        for domain_list, genome_in_domain, ng_size in zip(just_neigh_data, genomes_with_domain, neigh_genome_size):
            ...
            if counter != 100:
                ...
                # pdb.set_trace()
                for fine_domain in alls:
                    dlok = str(neigh_domains_counter[fine_domain] / int(ng_size[0]))
                    dglob = str(genome_domains_counter[fine_domain] / int(ng_size[1]))
                    # diff = dlok - dglob
                    part.append((genome_in_domain, fine_domain,dglob))
                big_data.append(part)
            ...
            if counter != 100 and domain_list == just_neigh_data[-1]:
                ...
                for place, domain in enumerate(alls):
                    data_to_save = [x[place] for x in big_data]
                    with open('/home/djangoadmin/final_site-project/scripts/temp_data/all', 'a+') as output:
                        for num in data_to_save:
                            # if num != '0.0':
                            output.write(' '.join(num))
                            output.write("\n")
                big_data = []
                counter = 0
            elif counter == 100:
...
if __name__ == "__main__":
    ...
    a = SuperSpeedAnalysisFromDomain('pfam02696', 5000, 'escherichia', 'none' ,'none', 'both','/media/results/timing.txt' )
    a.go()
