...
class SuperSpeedAnalysisFromDomain:
    ...
    def go(self):
        ...
        #     neigh_counter += neigh_domains_counter
        #     temp_data = []
        #     with open('temp_data/temp_data'+str(counter), 'w') as output:
        #
        #         for fine_domain in alls:
        #
        #             dlok = neigh_domains_counter[fine_domain] / int(ng_size[0])
        #             dglob = genome_domains_counter[fine_domain] / int(ng_size[1])
        #             diff = dlok - dglob
        #             # temp_data.append(diff)
        #             output.write(str(diff))
        #             output.write("\t\n")
        #         output.close()
        #         counter +=1
        #         # file = "temp_data/{}".format(fine_domain)
        #         # command = "echo {} >> {}".format(diff, file)
        #         # os.system(command)
        ...
        #         dglob = genome_domains_counter[fine_domain] / int(ng_size[1])
        #         diff = dlok - dglob
        #         with open('temp_data/'+ fine_domain, 'a') as output:
        #             output.write(str(diff))
        #             output.write("\n")
        #             output.close()
        ...
        "CORRECT WAY OF WRITE HOWEVER LATER TOO LONG TO READ"
        # with open('testowy_zapis.txt', 'w') as output:
        #
        #     for domain_list,genome_in_domain,ng_size in zip(just_neigh_data,genomes_with_domain,neigh_genome_size):
        #
        #         genes, start_coord, end_coord, orientation, domains, contig = self.create_six_list(genome_in_domain)
        ...
        "SECOND WAY OF READING, NEEDS TO VALIDATE"
        # scores = []
        # for i in range(0,counter+1):
        #     lista_danych_to_wilcox_pre = self.open_multiple_line("temp_data/"+str(i),split_mark ="\t")
        #     lista_danych_to_wilcox = [float(x) for x in lista_danych_to_wilcox_pre]
        #     wynik = test(lista_danych_to_wilcox, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(lista_danych_to_wilcox)))


        "OTHER WAY TO CALCULATE"
        # scores = []
        # for i in range(0,counter +1):
        #     command = "cat temp_data/"+str(i)
        #     proc = subprocess.check_output(command, shell =True).decode('ascii')
        #     lista_danych_to_wilcox_pre = proc.strip().split('\n')
        #     lista_danych_to_wilcox = [float(x) for x in lista_danych_to_wilcox_pre]
        #     wynik = test(lista_danych_to_wilcox, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(lista_danych_to_wilcox)))


        "CORRECT WAY OF READING HOWEVER LATER TOO LONG "
        # scores = []
        # for i in range(1,len(alls)+1):
        #
        #     command = "awk '{print$" + str(i)+"}' testowy_zapis.txt"
        #     print(i)
        #     proc = subprocess.check_output(command, shell=True).decode('ascii')
        #     lista_danych_to_wilcox_pre = proc.strip().split('\n')
        #     lista_danych_to_wilcox = [float(x) for x in lista_danych_to_wilcox_pre ]
        ...
        # scores = []
        # max_files = len(genomes_with_domain)
        # max_domains = len(alls)
        #
        # lista_domen = list(range(0, max_domains + 1, 100))
        # lista_plikow = list(range(0, max_files))
        #
        # for i in lista_domen:
        #     print(i)
        #     setki = []
        #     for j in lista_plikow:
        #         testowe = self.open_multiple_line_two("temp_data/temp_data" + str(j))
        #         if i != lista_domen[-1]:
        #             setki.append(testowe[i:i + 100])
        #         else:
        #             y = max_domains - lista_domen[-1] + 1
        #             setki.append((testowe[i:i + y]))
        #     # counter = 0
        #     for k in range(0, 100):
        #         # counter +=1
        #         # print(counter)
        #         try:
        #             part = [float(x[k]) for x in setki]
        #
        #             wynik = test(part, zero_method="wilcox")
        #             scores.append((wynik.pvalue, statistics.mean(part)))
        #         except IndexError:
        #             continue
    ...
    def start(self):
        ...
        # for i in range(0,max_domains+1):
        #     print(i)
        #     pojedyncza_domena = []
        #     for j in range(0,max_files+1):
        #         testowe = self.open_multiple_line_two("temp_data/temp_data" + str(j))
        #         # print(len(testowe))
        #         pojedyncza_domena.append(float(testowe[i]))
        #     wynik = test(pojedyncza_domena, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(pojedyncza_domena)))
        # print("Counting", datetime.datetime.now())
        ...
        #     for j in range(0,max_files+1):
        #         testowe = self.open_multiple_line_two("temp_data/temp_data" + str(j))
        #         one.append(float(testowe[i]))
        #         two.append(float(testowe[i+1]))
        #         three.append(float(testowe[i+2]))
        #         four.append(float(testowe[i+3]))
        #         five.append(float(testowe[i+4]))
        #         six.append(float(testowe[i+5]))
        #         seven.append(float(testowe[i+6]))
        #         eight.append(float(testowe[i+7]))
        #         nine.append(float(testowe[i+8]))
        #         ten.append(float(testowe[i+9]))
        #     wynik = test(one, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(one)))
        #     wynik = test(two, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(two)))
        #     wynik = test(three, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(three)))
        #     wynik = test(four, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(four)))
        #     wynik = test(five, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(five)))
        #     wynik = test(six, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(six)))
        #     wynik = test(seven, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(seven)))
        #     wynik = test(eight, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(eight)))
        #     wynik = test(nine, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(nine)))
        #     wynik = test(ten, zero_method="wilcox")
        #     scores.append((wynik.pvalue, statistics.mean(ten)))
        # print("Counting", datetime.datetime.now())
        ...
        lista_domen = list(range(0, max_domains+1, 100))
        lista_plikow = list(range(0, max_files + 1))
