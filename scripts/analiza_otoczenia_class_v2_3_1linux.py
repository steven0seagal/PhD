"""
Script created Friday August 2nd 2019

@uthor: BP Baranowsky
"""

"""
In this script few crucial improvment were made:
--> When scores are calculated they immidiately go to DataFrame
--> Domain is counted as important when difference of local density and global density is bigger than 0
--> At the end add to DataFrame short description about domains !!! 
"""
"""
Thing to add:
...
class NeighbourhoodAnalyzer():
    ...
    def SaveComplete(self,file, message_for_output,message_for_additional_data, message_down,complete_data):
        ...
        # Vultr
        # with open('/home/djangoadmin/final_site-project/media/'+file+'.txt' , 'w') as output_file:
        # Local
        with open('/mnt/d/45.76.38.24/final_site-project'+file+'.txt', 'w') as output_file:
...
#        neighbourhood_complete.append(party)
    ...
    def CollectAllData(self,filter_scores, genome_dataframe, neigh_dataframe, 
        ...
        for pfam in filter_scores:
            ...
            if domena in avaible_domains:
                ...
                alternative_complete.at[domena, 'average occurence in neighbourhood'] = sum_neigh / genome_number_overall
                ...
                alternative_complete.at[domena,  'min occurence in genome'] = np.min([x for x in list(genome_dataframe[domena]) if str(x) != 'nan'])
                alternative_complete.at[domena, 'max occurence in genome'] = np.max([x for x in list(genome_dataframe[domena]) if str(x) != 'nan'])
            
        for add_data in filter_add_scores_avg_DLOK:
            DLOK_pfam = add_data[0]
            DLKO_pvalue = format(add_data[1], ".3e")
            if DLOK_pfam in avaible_domains:
                alternative_complete.at[DLOK_pfam, 'Local density'] = DLKO_pvalue
        
        for add_data in filter_add_scores_avg_DGLOB:
            DGLOB_pfam = add_data[0]
            DGLOB_pvalue = format(add_data[1], ".3e")
            if DGLOB_pfam in avaible_domains:
                alternative_complete.at[DGLOB_pfam, 'Global density'] = DGLOB_pvalue
    ...
    def MultipleTestCorrection(self,some_dataframe, correction_met):
        ...
        if correction_met == 'none':
            ...
            reject, pvals_corrected, alphaSidak, alphaBonf = correction(pvals=value_to_correct, method=correction_met,
    ...
    def GO(self):
        ...
        mighty_domains= self.OpenDirectory('/mnt/d/45.76.38.24/final_site-project/important_files/LISTA_DOMEN')
        GENOME_ID_SIZE_IN_BP = self.OpenFile('/mnt/d/45.76.38.24/final_site-project/important_files/GENOME_ID_SIZE_IN_BP')
        GENOME_ID_SIZE_IN_GENE = self.OpenFile('/mnt/d/45.76.38.24/final_site-project/important_files/GENOME_ID_SIZE_IN_GENE')
        ...
        message_for_additional_data = "Pfam domain , PVALUE,  occurence in neighbourhoods, average occurence in neighbourhood ,min occurence in neighbourhood , max occurence in neighbourhood ,occurence genomes, average occurence in genome, min occurence in genome,  max occurence in genome, avg DLOK-DGLOB, avg DLOK, avg DGLOB, Family, Summary"
        ...
        """
       # Local
        file_names = self.OpenDirectory('/mnt/d/45.76.38.24/final_site-project/important_files/'+ tax)
        ...
        for file in file_names[:100]:
            ...
            for point in coords:
                ...
                self.DataForWilcoxonTest(D_LOK,D_GLOB,matrix,file,dlok_matrix, dglob_matrix)
        ...
        alternative_complete = self.CollectAllData(filter_scores, genome_dataframe, 
                                            ...
                                            filter_add_scores_avg_DLOK,
                                            filter_add_scores_avg_DGLOB,
                                            filter_add_scores_avg_DLOK_DGLOB)
        ...
        # Vultr
        # domain_information = self.OpenInformationBoutDomains('/home/djangoadmin/final_site-project/important_files/domain_information')
        # Local
        domain_information =self.OpenInformationBoutDomains('/mnt/d/45.76.38.24/final_site-project/important_files/domain_information')
